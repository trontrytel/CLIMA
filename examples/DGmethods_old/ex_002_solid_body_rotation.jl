...
function velocity_initilization!(uvec::MVector{num_aux_states,FT}, x, y, z) where {FT}
    @inbounds begin
        r = hypot(x, y)
        θ = atan(y, x)
...
function upwindboundaryflux!(fs, nM, stateM, viscM, uvecM, stateP, viscP, uvecP, bctype, t)
    FT = eltype(fs)
    @inbounds begin
    ## determine the advection speed and direction
        un = dot(nM, uvecM)
