...
function DGBalanceLaw(;
    ...
    (Topologies.hasboundary(topology) &&
     numerical_boundary_flux! === nothing &&
     error("no `numerical_boundary_flux!` given when topology " * "has boundary"))
    ...
    # These should all be true in this case
        ...
        (Topologies.hasboundary(topology)) && (@assert viscous_boundary_penalty! !== nothing)
    end
...
"""
function MPIStateArrays.MPIStateArray(disc::DGBalanceLaw, ic!::Function; commtag = 888)
...
"""
function indefinite_stack_integral!(disc::DGBalanceLaw, f, Q, out_states, P = disc.auxstate)
    grid = disc.grid
    topology = grid.topology
