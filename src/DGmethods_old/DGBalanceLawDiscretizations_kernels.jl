                    ...
                    @unroll for s = 1:nstate
                        ...
                        s_F[1, i, j, k, s] = l_M[i, j, k] *
                                             (l_ξ1x1[i, j, k] * F1 + l_ξ1x2[i, j, k] * F2 +
                                              l_ξ1x3[i, j, k] * F3)
                        s_F[2, i, j, k, s] = l_M[i, j, k] *
                                             (l_ξ2x1[i, j, k] * F1 + l_ξ2x2[i, j, k] * F2 +
                                              l_ξ2x3[i, j, k] * F3)
                        s_F[3, i, j, k, s] = l_M[i, j, k] *
                                             (l_ξ3x1[i, j, k] * F1 + l_ξ3x2[i, j, k] * F2 +
                                              l_ξ3x3[i, j, k] * F3)
                    end
                end
            end
...
function facerhs!(
    ...
    @inbounds @loop for e in (elems; blockIdx().x)
        ...
        # Load plus side data
                ...
                bctype = numerical_boundary_flux! === nothing ? 0 : elemtobndy[f, e]
                if bctype == 0
                    numerical_flux!(
                        l_F,
                        nM,
...
function faceviscterms!(
    ...
    @inbounds @loop for e in (elems; blockIdx().x)
        ...
        # Load plus side data
                ...
                bctype = viscous_boundary_penalty! === nothing ? 0 : elemtobndy[f, e]
                if bctype == 0
                    viscous_penalty!(l_Qvisc, nM, l_GM, l_QM, l_auxM, l_GP, l_QP, l_auxP, t)
                else
                    viscous_boundary_penalty!(
                        l_Qvisc,
                        nM,
            ...
            # ξ3-grid lines
                    ...
                    @unroll for d = 1:3
                        l_fd[d, i, j, k] = l_ξ1d[d, i, j, k] * fξ1 +
                                           l_ξ2d[d, i, j, k] * fξ2 + l_ξ3d[d, i, j, k] * fξ3
                    end
                end
            end
        end
