...
function compute_diagnosticsums!(FT, state, aux, i, j, k, ijk, ev, eh, e, x, y, z,
    ...
    # vertical fluxes
    ds.vert_eddy_mass_flx += rep * (w - w̃) * (state.ρ - ha.ρ)
    ds.vert_eddy_u_flx    += rep * (w - w̃) * (u - ha.ρu / ha.ρ)
    ds.vert_eddy_v_flx    += rep * (w - w̃) * (v - ha.ρv / ha.ρ)
    ds.vert_eddy_qt_flx   += rep * (w - w̃) * (q_tot - q̃_tot)
    ds.vert_qt_flx        += rep * w * q_tot
    ds.vert_eddy_ql_flx   += rep * (w - w̃) * (th.q_liq - ha.q_liq)
    ds.vert_eddy_qv_flx   += rep * (w - w̃) * (th.q_vap - ha.q_vap)
    ds.vert_eddy_thd_flx  += rep * (w - w̃) * (th.θ_dry - ha.θ_dry)
    ds.vert_eddy_thv_flx  += rep * (w - w̃) * (th.θ_v - ha.θ_v)
    ds.vert_eddy_thl_flx  += rep * (w - w̃) * (th.θ_liq_ice - ha.θ_liq_ice)

    # variances
    ds.uvariance += rep * (u - ũ)^2
    ds.vvariance += rep * (v - ṽ)^2
    ds.wvariance += rep * (w - w̃)^2

    # skewness
    ds.wskew     += rep * (w - w̃)^3

    # turbulent kinetic energy
    ds.TKE       += rep * 0.5 * (ds.uvariance + ds.vvariance + ds.wvariance)
end
