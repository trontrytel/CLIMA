...
abstract type AbstractGrid{FloatType,dim,polynomialorder,numberofDOFs,DeviceArray} end
...
struct DiscontinuousSpectralElementGrid{
    ...
    function DiscontinuousSpectralElementGrid(
     ...
     # FIXME: There has got to be a better way!
        ...
        new{FloatType,dim,N,Np,DeviceArray,DAT1,DAT2,DAT3,DAT4,DAI1,DAI2,DAI3,TOP}(
            topology,
            vgeo,
            sgeo,
            elemtobndy,
...
function Base.propertynames(G::DiscontinuousSpectralElementGrid)
    (fieldnames(DiscontinuousSpectralElementGrid)..., keys(vgeoid)..., keys(sgeoid)...)
end
