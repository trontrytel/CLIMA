...
"""
function centroidtocode(comm::MPI.Comm, elemtocorner; coortocode = hilbertcode, CT = UInt64)
...
function brickmesh(
    ...
    fmask = hcat((p[ntuple(
        j -> (j == div(f - 1, 2) + 1) ? (mod(f - 1, 2)+1:mod(f - 1, 2)+1) : (:),
        d,
    )...,][:] for f = 1:nface)...)
    ...
    for (e, ec) in enumerate(elems[elemlocal])
        ...
        for i = 1:d
            if periodic[i] && ec[i] == nelemdim[i]
                ...
                push!(faceconnections, vcat(e, 2i, verts[neighcorners[fmask[:, 2i-1]]]))
            end
        end
    end
...
function parallelsortcolumns(
  ...
  # if we don't have any pivots then we must have zero columns
    ...
    pivots = [pivots[i, div(div(length(pivots), m) * r, csize)+1] for i = 1:m, r = 0:csize-1]

    cols = map(i -> view(A, :, i), 1:n)
    sendstarts = [(i <= csize) ?
                  (searchsortedfirst(cols, pivots[:, i], lt = lt, by = by, rev = rev) - 1) *
                  m + 1 :
                  n * m + 1 for i = 1:csize+1]
...
"""
function partition(comm::MPI.Comm, elemtovert, elemtocoord, elemtobndy, faceconnections)
  ...
  #
    elemtofaceconnect = zeros(eltype(eltype(faceconnections)), nfacevert, nface, nelem)
    ...
    for r = 0:csize-1
        ...
        netv = MPI.Gatherv(view(elemtovert, :, sendrange), rcounts .* Cint(nvert), r, comm)
        ...
        netb = MPI.Gatherv(view(elemtobndy, :, sendrange), rcounts .* Cint(nface), r, comm)
    ...
    newelemtofaceconnect = reshape(newelemtofaceconnect, nfacevert, nface, newnelem)
...
function connectmesh(
  ...
  # Mark which faces need to be sent
    ...
    nabrtosendrank = Int[r for r = 0:csize-1 if sendstarts[r+2] - sendstarts[r+1] > 0]
    nabrtosend = UnitRange{Int}[(sendstarts[r+1]:(sendstarts[r+2]-1)) for r = 0:csize-1 if sendstarts[r+2] -
                                                                                           sendstarts[r+1] > 0]
  ...
  # Mark which faces will be received
    ...
    nabrtorecvrank = Int[r for r = 0:csize-1 if recvstarts[r+2] - recvstarts[r+1] > 0]
    nabrtorecv = UnitRange{Int}[(recvstarts[r+1]:(recvstarts[r+2]-1)) for r = 0:csize-1 if recvstarts[r+2] -
                                                                                           recvstarts[r+1] > 0]
  ...
  # fill the ghost values in elemtocoord
    ...
    crreq = [MPI.Irecv!(view(newelemtocoord, :, :, nelem .+ er), r, 666, comm) for (
        r,
        er,
    ) in zip(nabrtorank, nabrtorecv)]

    brreq = [MPI.Irecv!(view(newelemtobndy, :, nelem .+ er), r, 666, comm) for (
        r,
        er,
    ) in zip(nabrtorank, nabrtorecv)]

    csreq = [MPI.Isend(view(sendelemtocoord, :, :, es), r, 666, comm) for (r, es) in zip(
        nabrtorank,
        nabrtosend,
    )]

    bsreq = [MPI.Isend(view(sendelemtobndy, :, es), r, 666, comm) for (r, es) in zip(
        nabrtorank,
        nabrtosend,
    )]
...
function mappings(N, elemtoelem, elemtoface, elemtoordr)
    ...
    fd(f) = div(f - 1, 2) + 1
    fe(f) = N * mod(f - 1, 2) + 1
    fmask = hcat((p[ntuple(j -> (j == fd(f)) ? (fe(f):fe(f)) : (:), d)...][:] for f = 1:nface)...)
...
function commmapping(N, commelems, commfaces, nabrtocomm)
    ...
    i = 1
    e = 1
    for neighbor = 1:length(nabrtocomm)
        rbegin = i
