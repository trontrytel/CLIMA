...
function Base.getproperty(a::StackedBrickTopology, p::Symbol)
    return p == :stacksize ? getfield(a, p) : getproperty(getfield(a, :topology), p)
end
...
function StackedBrickTopology(
  ...
  # Use the base topology to build the stacked topology
    ...
    sendelems = similar(basetopo.sendelems, length(basetopo.sendelems) * stacksize)
...
```
"""
function CubedShellTopology(mpicomm, Neside, T; connectivity = :face, ghostsize = 1)
...
function StackedCubedSphereTopology(
    ...
    # v is base vertex
        ...
        # 2^(dim-1) + v is higher stacked vertex
                elemtocoord[d, 2^(dim-1)+v, e] = basetopo.elemtocoord[d, v, i] * Rrange[j+1]
            end
        end
    end
