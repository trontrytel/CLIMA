...
@static if haspkg("CuArrays")
    using CuArrays
    Base.similar(::Type{CuArray}, ::Type{FT}, dims...) where {FT} =
...
struct MPIStateArray{
    ...
    # Better way than checking the type names?
        if typeof(vmaprecv).name != typeof(data).name
            vmaprecv = copyto!(similar(DA, eltype(vmaprecv), size(vmaprecv)), vmaprecv)
        end
        if typeof(vmapsend).name != typeof(data).name
            vmapsend = copyto!(similar(DA, eltype(vmapsend), size(vmapsend)), vmapsend)
        end
        ...
        new{FT,typeof(data),DAI1,DAV,DAT2}(
            mpicomm,
            data,
            realdata,
            realelems,
...
function Base.similar(Q::MPIStateArray, ::Type{FTN}; commtag = Q.commtag) where {FTN}
    MPIStateArray{FTN}(
        Q.mpicomm,
        Q.data,
        size(Q.data)...,
...
function Base.similar(bc::Broadcasted{ArrayStyle{MPIStateArray}}, ::Type{FT}) where {FT}
...
function post_Irecvs!(Q::MPIStateArray)
    ...
    # If this fails we haven't waited on previous recv!
        ...
        Q.recvreq[n] = MPI.Irecv!(
            (@view Q.host_recv_buffer[:, Q.nabrtovmaprecv[n]]),
            Q.nabrtorank[n],
            Q.commtag,
            Q.mpicomm,
        )
  ...
  # post MPI sends
    ...
    for n = 1:nnabr
        Q.sendreq[n] = MPI.Isend(
            (@view Q.host_send_buffer[:, Q.nabrtovmapsend[n]]),
            Q.nabrtorank[n],
            Q.commtag,
            Q.mpicomm,
        )
...
function _fillsendbuf!(sendbuf, buf, vmapsend)
    if length(vmapsend) > 0
        ...
        @launch(
            ...
            threads = threads,
            blocks = blocks,
            knl_fillsendbuf!(Val(Np), Val(nvar), sendbuf, buf, vmapsend, length(vmapsend))
        )
    end
end
...
LinearAlgebra.norm(Q::MPIStateArray, weighted::Bool; dims = nothing) =
...
function norm_impl(Q::SubArray{FT,N,A}, ::Val{p}, dims::Nothing) where {FT,N,A<:Array,p}
...
function norm_impl(Q, ::Val{p}, dims = nothing) where {p}
    ...
    dims == nothing ? mapreduce(f, op, Q) : mapreduce(f, op, Q, dims = dims)
end
...
function weighted_norm_impl(Q, W, ::Val{p}, dims = nothing) where {p}
    ...
    dims == nothing ? reduce(op, E, init = init) : reduce(op, E, init = init, dims = dims)
end
