...
function diffusive_boundary_penalty!(
    nf::CentralGradPenalty,
    bl::BalanceLaw,
    VF,
    nM,
    diffM,
    ...
    gradvariables!(
        bl,
        Vars{vars_gradient(bl, FT)}(diffP),
        Vars{vars_state(bl, FT)}(QP),
        Vars{vars_aux(bl, FT)}(aP),
        t,
    )
...
function numerical_flux_nondiffusive!(
    ...
    λM = wavespeed(bl, nM, Vars{vars_state(bl, FT)}(QM), Vars{vars_aux(bl, FT)}(auxM), t)
    FM = similar(F, Size(3, nstate))
    fill!(FM, -zero(eltype(FM)))
    flux_nondiffusive!(
        bl,
        Grad{vars_state(bl, FT)}(FM),
        Vars{vars_state(bl, FT)}(QM),
        Vars{vars_aux(bl, FT)}(auxM),
        t,
    )

    λP = wavespeed(bl, nM, Vars{vars_state(bl, FT)}(QP), Vars{vars_aux(bl, FT)}(auxP), t)
    FP = similar(F, Size(3, nstate))
    fill!(FP, -zero(eltype(FP)))
    flux_nondiffusive!(
        bl,
        Grad{vars_state(bl, FT)}(FP),
        Vars{vars_state(bl, FT)}(QP),
        Vars{vars_aux(bl, FT)}(auxP),
        t,
    )
...
function numerical_boundary_flux_diffusive!(
