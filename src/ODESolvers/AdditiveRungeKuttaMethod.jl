...
mutable struct AdditiveRungeKutta{T,RT,AT,LT,Nstages,Nstages_sq} <: ODEs.AbstractODESolver
    ...
    function AdditiveRungeKutta(
        ...
        new{T,RT,AT,LT,nstages,nstages^2}(
            RT(dt),
            RT(t0),
            rhs!,
            rhs_linear!,
...
function AdditiveRungeKutta(
    ...
    rhs_linear! = (x...; increment) -> SpaceMethods.odefun!(
        spacedisc_linear,
        x...,
        increment = increment,
    )
...
function ODEs.dostep!(Q, ark::AdditiveRungeKutta, p, timeend::Real, adjustfinalstep::Bool)
