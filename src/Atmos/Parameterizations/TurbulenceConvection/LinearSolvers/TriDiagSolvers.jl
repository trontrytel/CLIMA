...
function TridiagonalMatrix(
    ...
    b_ = [1 + Δt * grid.Δzi2 *
          (sum((tmp[ρ, Dual(k)] .* q[a, Dual(k), i] .* tmp[K, Dual(k), i]))) for k in over_elems_real(grid)]
...
function solve_tdma!(
    ...
    B = [tmp[a_τ, k, i] / q[a_τp1, k, i] * q[x, k, i] +
         Δt * tendencies[x, k, i] / (tmp[ρ, k] * q[a_τp1, k, i]) for k in over_elems_real(grid)]
