...
function StateVec(vars, grid::Grid{T}, dd::DomainDecomp) where {T}
    ...
    all_vars = Any[eltype(grid.Δz)(0) for v = 1:n_vars]
...
function Base.show(io::IO, sv::StateVec)
    ...
    vn = var_names(sv)
    headers = [length(over_sub_domains(sv, ϕ)) == 1 ? string(ϕ) :
               string(ϕ) * "_" * string(i) for ϕ in vn for i in over_sub_domains(sv, ϕ)]
    n_vars = length(headers)
    n_elem = length(sv.fields)
    data = reshape(
        [sv[ϕ, k, i] for ϕ in vn for i in over_sub_domains(sv, ϕ) for k = 1:n_elem],
        n_elem,
        n_vars,
    )
...
function Base.getindex(sv::StateVec, ϕ::Symbol, cut::Cut, i = 0)
    @inbounds [sv[ϕ, k, i] for k = cut.e-1:cut.e+1]
end
...
function Base.getindex(sv::StateVec, ϕ::Symbol, dual::Dual, i = 0)
    @inbounds [(sv[ϕ, k, i] + sv[ϕ, k+1, i]) / 2 for k = dual.e-1:dual.e]
end
