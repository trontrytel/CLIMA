...
function flux_radiation!(m::StevensRadiation, flux::Grad, state::Vars, aux::Vars, t::Real)
    FT = eltype(flux)
  ...
  # Constants
    ...
    free_troposphere_cooling = m.ρ_i * FT(cp_d) * m.D_subsidence * m.α_z *
                               ((cbrt(Δz_i))^4 / 4 + m.z_i * cbrt(Δz_i))
    F_rad = cloud_base_warming + cloud_base_warming + free_troposphere_cooling
    flux.ρe += SVector(FT(0), FT(0), F_rad)
end
function preodefun!(m::StevensRadiation, aux::Vars, state::Vars, t::Real) end
