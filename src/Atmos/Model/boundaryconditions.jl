...
function atmos_boundary_state!(
    ...
    atmos_boundary_state!(nf, bctup[bctype], m, stateP, auxP, nM, stateM, auxM, bctype, t)
end
...
function atmos_boundary_state!(
  ...
  # Assign reflection wall boundaries (top wall)
    stateP.ρu = SVector(UM - 2 * nM[1] * UnM, VM - 2 * nM[2] * UnM, WM - 2 * nM[3] * UnM)
...
function atmos_boundary_state!(
  ...
  # Assign diffusive fluxes at boundaries
    ...
    # (i.e. ρ𝛕 terms)  
        stateP.ρu = SVector(0, 0, 0)
        diffP.ρτ = SHermitianCompact{3,FT,6}(SVector(
            FT(0),
            ρτM[2, 1],
            ρτ13P,
            FT(0),
    ...
    # ----------------------------------------------------------
        diffP.moisture.ρd_q_tot = SVector(FT(0), FT(0), bc.LHF / (LH_v0))
    # ----------------------------------------------------------
    # Boundary energy fluxes
    # ----------------------------------------------------------
    # Assign diffusive enthalpy flux (i.e. ρ(J+D) terms) 
        diffP.ρd_h_tot = SVector(FT(0), FT(0), bc.LHF + bc.SHF)
    end
end
