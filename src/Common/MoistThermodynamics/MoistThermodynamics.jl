...
"""
gas_constant_air(ts::ThermodynamicState) = gas_constant_air(PhasePartition(ts))
...
linearized_air_pressure(
    ...
    ρ * FT(R_d) * FT(T_0) +
    FT(R_d) / FT(cv_d) * (ρ * e_tot - ρ * e_pot - (ρ * q.tot - ρ * q.liq) * FT(e_int_v0) +
     ρ * q.ice * (FT(e_int_i0) + FT(e_int_v0)))

linearized_air_pressure(e_kin::FT, e_pot::FT, ts::ThermodynamicState{FT}) where {FT<:Real} =
    linearized_air_pressure(
        air_density(ts),
        total_energy(e_kin, e_pot, ts),
        e_pot,
        PhasePartition(ts),
    )
...
"""
specific_volume(ts::ThermodynamicState) =
    specific_volume(air_temperature(ts), air_pressure(ts), PhasePartition(ts))
specific_volume(ts::PhaseDry) =
    (gas_constant_air(ts) * air_temperature(ts)) / air_pressure(ts)
...
cp_m(q::PhasePartition{FT}) where {FT<:Real} =
    FT(cp_d) + (FT(cp_v) - FT(cp_d)) * q.tot + (FT(cp_l) - FT(cp_v)) * q.liq +
...
cv_m(q::PhasePartition{FT}) where {FT<:Real} =
    FT(cv_d) + (FT(cv_v) - FT(cv_d)) * q.tot + (FT(cv_l) - FT(cv_v)) * q.liq +
...
"""
air_temperature(ts::ThermodynamicState) =
    air_temperature(internal_energy(ts), PhasePartition(ts))
air_temperature(ts::PhaseDry{FT}) where {FT<:Real} =
    FT(T_0) + internal_energy(ts) / cv_m(ts)
...
internal_energy(T::FT, q::PhasePartition{FT} = q_pt_0(FT)) where {FT<:Real} =
    cv_m(q) * (T - FT(T_0)) + (q.tot - q.liq) * FT(e_int_v0) -
...
"""
internal_energy_sat(ts::PhaseDry{FT}) where {FT<:Real} =
    cv_d * (air_temperature(ts) - FT(T_0))
...
total_energy(
    ...
    T::FT,
    q::PhasePartition{FT} = q_pt_0(FT),
) where {FT<:Real} = e_kin + e_pot + internal_energy(T, q)
...
given a thermodynamic state `ts`.
"""
latent_heat_vapor(ts::ThermodynamicState) = latent_heat_vapor(air_temperature(ts))
...
given a thermodynamic state `ts`.
"""
latent_heat_sublim(ts::ThermodynamicState) = latent_heat_sublim(air_temperature(ts))
...
"""
latent_heat_generic(T::FT, LH_0::FT, Δcp::FT) where {FT<:Real} = LH_0 + Δcp * (T - FT(T_0))
...
liquid_fraction_equil(ts::PhaseDry{FT}) where {FT<:Real} =
    FT(air_temperature(ts) > FT(T_freeze))
...
function liquid_ice_pottemp_given_pressure(
    ...
    return dry_pottemp_given_pressure(T, p, q) *
           (1 - latent_heat_liq_ice(q) / (cp_m(q) * T))
end
...
dry_pottemp_given_pressure(
    ...
    p::FT,
    q::PhasePartition{FT} = q_pt_0(FT),
) where {FT<:Real} = T / exner_given_pressure(p, q)
...
function air_temperature_from_liquid_ice_pottemp(
    ...
    cv = cv_m(q)
    R_m = gas_constant_air(q)
    return θ_liq_ice * (ρ * R_m * θ_liq_ice / FT(MSLP))^(R_m / cv) +
           latent_heat_liq_ice(q) / cv
end
...
function air_temperature_from_liquid_ice_pottemp_non_linear(
    ...
    T, converged = find_zero(
        T -> T - air_temperature_from_liquid_ice_pottemp_given_pressure(
            θ_liq_ice,
            air_pressure(T, ρ, q),
            q,
        ),
...
virtual_pottemp(ts::PhaseDry{FT}) where {FT<:Real} =
    gas_constant_air(ts) / FT(R_d) * dry_pottemp(ts)
...
liquid_ice_pottemp_sat(ts::PhaseDry) = liquid_ice_pottemp(
    air_temperature(ts),
    air_density(ts),
    PhasePartition(q_vap_saturation(ts)),
)
...
"""
exner(ts::ThermodynamicState) =
    exner(air_temperature(ts), air_density(ts), PhasePartition(ts))
exner(ts::PhaseDry{FT}) where {FT<:Real} =
    (air_pressure(ts) / FT(MSLP))^(gas_constant_air(ts) / cp_m(ts))
...
"""
relative_humidity(ts::ThermodynamicState) =
    air_pressure(ts) / saturation_vapor_pressure(ts, Liquid())
