...
@testset "moist thermodynamics - correctness" begin
  ...
  # speed of sound
    @test soundspeed_air(T_0 + 20, PhasePartition(FT(0))) == sqrt(cp_d / cv_d * R_d *
                                                                  (T_0 + 20))
    @test soundspeed_air(T_0 + 100, PhasePartition(FT(1))) == sqrt(cp_v / cv_v * R_v *
                                                                   (T_0 + 100))
  ...
  # saturation vapor pressure and specific humidity
    ...
    @test saturation_excess(FT(T_triple), ρ, PhasePartition(q_tot)) == q_tot -
                                                                       ρ_v_triple / ρ
  ...
  # energy functions and inverse (temperature)
    ...
    @test total_energy(FT(e_kin), FT(e_pot), FT(T)) ≈ FT(e_kin) + FT(e_pot) +
                                                      cv_d * (T - T_0)
  ...
  # potential temperatures
    ...
    @test liquid_ice_pottemp_given_pressure(T, FT(MSLP) / 10, PhasePartition(FT(1))) ≈ T *
                                                                                       10^(R_v /
                                                                                           cp_v)
...
@testset "moist thermodynamics - constructor consistency" begin
  ...
  # air_temperature_from_liquid_ice_pottemp_given_pressure-liquid_ice_pottemp inverse
    ...
    @test all(air_temperature_from_liquid_ice_pottemp_given_pressure.(
        θ_liq_ice,
        p,
        q_pt_dry,
    ) .≈ T)
  ...
  # Accurate but expensive `LiquidIcePotTempSHumNonEquil` constructor (Non-linear temperature from θ_liq_ice)
    T_non_linear = air_temperature_from_liquid_ice_pottemp_non_linear.(
        θ_liq_ice,
        ρ,
        q_pt_moist,
    )
  ...
  # LiquidIcePotTempSHumEquil: Moist case
    ...
    @test maximum(abs.(liquid_ice_pottemp.(ts) - θ_liq_ice)) ./ eps(FT) < sqrt(1 /
                                                                               (eps(FT)))
    @test all(air_density.(ts) .≈ ρ)
    @test all(getproperty.(PhasePartition.(ts), :tot) .≈ q_tot_moist)
  ...
  # PhasePartition test is noisy, so do this only once:
    ...
    for e_int in e_int_range
        for ρ in ρ_range
            ...
            @test linearized_air_pressure(e_kin, e_pot, ts_eq) ≈ linearized_air_pressure(
                e_kin,
                e_pot,
                ts_dry,
            )
            ...
            @test saturation_vapor_pressure(ts_eq, Ice()) ≈ saturation_vapor_pressure(
                ts_dry,
                Ice(),
            )
            @test saturation_vapor_pressure(ts_eq, Liquid()) ≈ saturation_vapor_pressure(
                ts_dry,
                Liquid(),
            )
