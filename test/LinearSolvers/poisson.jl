...
import CLIMA.DGmethods.NumericalFluxes: NumericalFluxDiffusive, numerical_flux_diffusive!
...
dxx_sol1d(x) = -16 * pi^2 * sin(2pi * x)^2 * (sin(2pi * x)^2 - 3 * cos(2pi * x)^2)
...
let
    ...
    linmethods = (
        b -> GeneralizedConjugateResidual(3, b, tol),
