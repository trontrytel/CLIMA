...
const _a_x,
    ...
    _a_P,
    _a_T,
    _a_soundspeed_air = 1:_nauxstate
...
DoFstorage = (Nex * Ney * Nez) * (Npoly + 1)^numdims *
             (_nstate + _nviscstates + _nauxstate + CLIMA.Mesh.Grids._nvgeo) +
             (Nex * Ney * Nez) * (Npoly + 1)^(numdims - 1) * 2^numdims *
             (CLIMA.Mesh.Grids._nsgeo)
...
gradient_vars!(gradient_list, Q, aux, t, _...) =
    gradient_vars!(gradient_list, Q, aux, t, preflux(Q, ~, aux)...)
...
@inline function compute_stresses!(VF, grad_mat, _...)
    ...
    # FIXME: Grab functions from module SubgridScaleTurbulence 
        SijSij = S11^2 + S22^2 + S33^2 + 2 * S12^2 + 2 * S13^2 + 2 * S23^2
...
"""
@inline function stresses_boundary_penalty!(
    VF,
    nM,
    gradient_listM,
    QM,
...
@inline function stresses_penalty!(
    VF,
    nM,
    gradient_listM,
    QM,
...
function run(mpicomm, dim, Ne, N, timeend, FT, dt)
  ...
  # brickrange defines the domain extents
    @timeit to "Topo init" topl = StackedBrickTopology(
        mpicomm,
        brickrange,
        periodicity = (false, false),
    )

    @timeit to "Grid init" grid = DiscontinuousSpectralElementGrid(
        topl,
        FloatType = FT,
  ...
  # spacedisc = data needed for evaluating the right-hand side function
    @timeit to "Space Disc init" spacedisc = DGBalanceLaw(
        grid = grid,
        ...
        auxiliary_state_initialization! = (x...) -> auxiliary_state_initialization!(x...),
        source! = source!,
        preodefun! = preodefun!,
    )
  ...
  # This is a actual state/function that lives on the grid
    ...
    # Set up the information callback
        ...
        cbvtk = GenericCallbacks.EveryXSimulationSteps(1000) do (init = false)
            ...
            outprefix = @sprintf(
                "./CLIMA-output-scratch/stretching/dy_%dD_mpirank%04d_step%04d",
                dim,
...
let
    ...
    loglevel = ll == "DEBUG" ? Logging.Debug :
               ll == "WARN" ? Logging.Warn : ll == "ERROR" ? Logging.Error : Logging.Info
  ...
  # User defined polynomial order 
    ...
    if MPI.Comm_rank(mpicomm) == 0
        ...
        @info @sprintf """     Minimum necessary memory to run this test: %g GBs """ (DoFstorage *
                                                                                      sizeof(FT)) /
                                                                                     1000^3
    ...
    engf_eng0 = run(mpicomm, dim, numelem[1:dim], polynomialorder, timeend, FT, dt)
