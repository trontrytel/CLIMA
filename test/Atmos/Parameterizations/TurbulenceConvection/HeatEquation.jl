...
@testset "∂_t T = ∇ • (K(z) ∇T) + 1, T = {0, 0} ∈ {z_{min}, z_{max}}, comparison" begin
    ...
    k_star2 = first_interior(grid, Zmax())
    q = StateVec(
        ((:T_explicit_surf, dss), (:T_explicit_vol, dss), (:T_implicit, dss), (:a, dss)),
        grid,
        dd,
    )
    tmp = StateVec(
    ...
    for i = 1:maxiter
        for k in over_elems_real(grid)
            ...
            rhs[:T_implicit, k] = VS
            k == k_star1 && (rhs[:T_implicit, k] += bc_source(
                q,
                grid,
                tmp,
                :T_implicit,
            ...
            k == k_star2 && (rhs[:T_implicit, k] += bc_source(
                q,
                grid,
                tmp,
                :T_implicit,
            ...
            k == k_star1 && (rhs[:T_explicit_vol, k] += bc_source(
                q,
                grid,
                tmp,
                :T_explicit_vol,
            ...
            k == k_star2 && (rhs[:T_explicit_vol, k] += bc_source(
                q,
                grid,
                tmp,
                :T_explicit_vol,
