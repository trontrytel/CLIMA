...
@testset "RainFallSpeed" begin
  ...
  # some example values
    ...
    for q_rai in q_rain_range
        @test terminal_velocity(q_rai, ρ_air) ≈ terminal_velocity_empir(
            q_rai,
            q_tot,
            ρ_air,
            ρ_air_ground,
...
@testset "RainAccretion" begin
  ...
  # some example values
    ...
    for q_rai in q_rain_range
        @test conv_q_liq_to_q_rai_accr(q_liq, q_rai, ρ_air) ≈ accretion_empir(
            q_rai,
            q_liq,
            q_tot,
        ) atol = 0.1 * accretion_empir(q_rai, q_liq, q_tot)
    end
end
...
@testset "RainEvaporation" begin
  ...
  # example values
    ...
    for q_rai in q_rain_range
        @test conv_q_rai_to_q_vap(q_rai, q, T, p, ρ) ≈ rain_evap_empir(q_rai, q, T, p, ρ) atol = -0.5 *
                                                                                                 rain_evap_empir(
            q_rai,
            q,
            T,
