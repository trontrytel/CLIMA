...
function kinematic_model_nodal_update_aux!(
    ...
    aux.snow_w = FT(0) #terminal_velocity(aux.q_rai, state.ρ)
...
@inline function wavespeed(
    ...
    snow_w = FT(0) #terminal_velocity(state.ρq_rai / state.ρ, state.ρ)
    nu = nM[1] * u[1] + nM[3] * max(u[3], rain_w, snow_w, u[3] - rain_w, u[3] - snow_w)
...
@inline function flux_nondiffusive!(
    ...
    flux.ρq_rai = SVector(
        state.ρu[1] * state.ρq_rai / state.ρ,
        FT(0),
        (state.ρu[3] / state.ρ - snow_w) * state.ρq_rai,
    )
    # ... energy ...
    flux.ρe = SVector(
...
function main()
    ...
    θ_0 = FT(282) # init. theta value (const) [K]
    p_0 = FT(101500) # surface pressure [Pa]
    p_1000 = FT(100000) # reference pressure in theta definition [Pa]
    qt_0 = FT(4.5 * 1e-3) # init. total water specific humidity (const) [kg/kg]
    z_0 = FT(0) # surface height

    # time stepping
    t_ini = FT(0)
    t_end = FT(.5*60) #FT(30 * 60)
    dt = FT(1) #FT(5)
    #CFL = FT(1.75)
    filter_freq = 1
    output_freq = 1 #72
