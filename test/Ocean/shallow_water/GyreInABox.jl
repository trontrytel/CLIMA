...
import CLIMA.ShallowWater: shallow_init_state!,
                           shallow_init_aux!,
                           vars_state,
                           vars_aux,
...
function null_init_state!(::GyreInABox, ::TurbulenceClosure, state, aux, coord, t)
    T = eltype(state.U)
...
function lsw_init_state!(p::GyreInABox, ::TurbulenceClosure, state, aux, coords, t)
    state.U = @SVector [u_lsw(coords[1], coords[2], t), v_lsw(coords[1], coords[2], t), 0]
...
function lkw_init_state!(p::GyreInABox, ::TurbulenceClosure, state, aux, coords, t)
    state.U = @SVector [u_lkw(coords[1], coords[2], t), v_lkw(coords[1], coords[2], t), 0]
...
R₂(x₁, ϵ) =
    (1 / D(ϵ)) * (((R₊(ϵ) * (exp(R₋(ϵ)) - 1)) * exp(R₊(ϵ) * x₁) +
      (R₋(ϵ) * (1 - exp(R₊(ϵ)))) * exp(R₋(ϵ) * x₁)) / (exp(R₊(ϵ)) - exp(R₋(ϵ))))
R₁(x₁, ϵ) =
    (π / D(ϵ)) * (1 .+
     ((exp(R₋(ϵ)) - 1) * exp(R₊(ϵ) * x₁) .+ (1 - exp(R₊(ϵ))) * exp(R₋(ϵ) * x₁)) /
     (exp(R₊(ϵ)) - exp(R₋(ϵ))))
...
ℋ(x₁, y₁, ϵ, βᵖ, fₒ, γ) =
    ...
    (R₁(x₁, ϵ) / π) * (sin(π * y₁) * (1.0 + βᵖ * (y₁ - 0.5)) + (βᵖ / π) * cos(π * y₁))
...
function gyre_init_state!(p::GyreInABox, V::ConstantViscosity, state, aux, coords, t)
    T = eltype(state.U)
