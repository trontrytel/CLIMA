...
# --------------- Driver definition ------------------ # 
function run(mpicomm, ArrayType, topl, dim, Ne, polynomialorder, timeend, FT, dt, model)
  # -------------- Define grid ----------------------------------- # 
  ...
  # Set up the information callback (output field dump is via vtk callback: see cbinfo)
    ...
    cbvtk = GenericCallbacks.EveryXSimulationSteps(3000) do (init = false)
        mkpath("./vtk-rb/")
        outprefix = @sprintf(
            "./vtk-rb/RB_%dD_mpirank%04d_step%04d",
            dim,
...
let
    ...
    @testset "$(@__FILE__)" for ArrayType in ArrayTypes
        ...
        Expected = (
            FT(9.9859344959259033e-01),
            FT(1.0038942098617554e+00),
            FT(1.0027571916580200e+00),
        )
