...
function run1(mpicomm, ArrayType, dim, topl, N, timeend, FT, dt)
  ...
  T_s = 320.0
  RH = 0.01
  model = AtmosModel{FT}(;ref_state=HydrostaticState(IsothermalProfile(T_s), RH),
                             source=nothing,
...
function run2(mpicomm, ArrayType, dim, topl, N, timeend, FT, dt)
  ...
  RH = 0.01
  model = AtmosModel{FT}(;ref_state=HydrostaticState(LinearTemperatureProfile(T_min, T_s, Γ), RH),
                         init_state=init_state!)
