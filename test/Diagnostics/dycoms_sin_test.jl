...
function config_dycoms(FT, N, resolution, xmax, ymax, zmax)
    config = CLIMA.LES_Configuration("Dycoms SIN test", N, resolution, xmax, ymax, zmax,
                                     init_sin_test!,
                                     solver_type=CLIMA.ExplicitSolverType(LSRK54CarpenterKennedy))
...
function main()
    ...
    Diagnostics.init(mpicomm, solver_config.dg, solver_config.Q, starttime, outdir)
